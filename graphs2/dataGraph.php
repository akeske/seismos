...
//	include("data.php");
	$k=$_GET['k2'];
	//graph area
	//$sqlgraph = $_SESSION['sql'.$k];
	$sqlgraph = $_SESSION['sqlgraphD2'.$k];
	$resultgraph = mysql_query($sqlgraph);
	$rowgraph = mysql_fetch_array($resultgraph);
	...
	$i=$i+1;
	$counter=1;
	for($j=$lastyear+1; $j<=$_SESSION['toyear2']; $j++) {
		if($flag==true) {
			if($counter==1){
				$xyear[$y]=$j;
				$ar[$i]=0;
